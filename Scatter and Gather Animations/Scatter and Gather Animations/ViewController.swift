...
//
//  Created by Gladymir Philippe on 7/25/20.
...
class ViewController: UIViewController {
    ...
    func configureLambda() {
        ...
        lambda.addArrangedSubview(self.letterA1)
    }
    ...
    @objc func fadeAndScatter() {
        let scatterBlock = {
            ...
            UIView.addKeyframe(withRelativeStartTime: 0.0, relativeDuration: 0.5, animations: {
                // Could do this with a CGAffineTransform (translation) as well
                self.letterA1.center = CGPoint(x: self.generateRandomNumber(), y: self.generateRandomNumber())
                self.letterA1.transform = CGAffineTransform(rotationAngle: self.randomAngle())
            })
        }
    ...
    @objc func fadeAndGather() {
        let gatherBlock = {
            ...
            UIView.addKeyframe(withRelativeStartTime: 0.0, relativeDuration: 0.5, animations: {
                // Could do this with a CGAffineTransform (translation) as well
                self.letterA1.transform = .identity
            })
